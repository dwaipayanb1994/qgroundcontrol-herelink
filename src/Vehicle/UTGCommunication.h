...
class UTGCommunication : public QObject
{
    ...
    Q_PROPERTY(bool continuousMeasurement READ continuousMeasurement NOTIFY continuousMeasurementChanged)
...
public:
    ...
    bool continuousMeasurement() const { return _continuousMeasurement; }
...
public slots:
    ...
    Q_INVOKABLE void setSettingsPushDeferred(bool deferred);
    Q_INVOKABLE void setNextReadingNotes(const QString& notes);
...
signals:
    ...
    void readingSaved();
    void continuousMeasurementChanged(bool measuring);
    ...
    void velocityApplied(double velocity);
...
private:
    ...
    bool _deferSettingsPush;
    QString _nextReadingNotes;

    // Velocity sync state
    bool _syncingFromDevice;
    double _lastSentVelocity;
    double _pendingVelocity;

    // Response buffer for partial messages
    QString _responseBuffer;

    // Suppress duplicate processing when the same response arrives via SERIAL_CONTROL and STATUSTEXT
    QString _lastProcessedResponse;
    qint64 _lastProcessedResponseMs = 0;
};
